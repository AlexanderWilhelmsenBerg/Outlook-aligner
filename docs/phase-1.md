...
Default console and JSON output withholds event subject and location. Body, attendees, email contents, and Teams URLs are not extracted by the Phase 1 probe.

Event subjects/locations are shown only when `--include-details` is explicitly supplied.

## Build artifact

Every Phase 1 pull-request CI run publishes a self-contained Windows x64 single-file executable:
...
Run these on the real Windows/Classic Outlook profile before Phase 1 is considered complete:

- [ ] `--help` runs without starting/touching Outlook.
- [ ] Default probe completes successfully.
...
- [ ] A recurring meeting with future occurrences is reported only within the horizon.
...
- [ ] Default output does not print meeting subjects or locations.
- [ ] `--include-details` prints subjects/locations only when explicitly requested.
...
Do not begin the forwarding spike until this PR has been manually tested against the real three-account Classic Outlook profile and merged by the user.
